...
def branch_and_bound(order,x_obj,y_obj,x_end,y_end):
    ...
    while(1):
	    ...
	    print all_mat[-1].order
            # all_mat.remove(all_mat[min_i])
        else:
            min_cost=float('inf')
            for i in range(0,len(all_mat)):
                # print all_mat[i].order
                if len(all_mat[i].order)==length:
                    last_distance = find_distance(x_end, y_end, x_obj[all_mat[i].order[-1]], y_obj[all_mat[i].order[-1]])
...
def write_result(order_list, x_start, y_start, x_end, y_end):
    ...
    for i in range(0, len(order_list)):
        writer.writerow([i + 1, x_start, y_start, x_end, y_end, ori_order[i], ori_dis[i], opt_order[i], opt_dis[i],mst_bound[i],bab_order[i],bab_dis[i]])
    csvfile.close()
